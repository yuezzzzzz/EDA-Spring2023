...
matrix1[4, ] #
matrix1[ , 3] #
matrix1[c(12, 14)] #the 12th and 14th value of matrix1.
matrix1[c(12:14)] #the 12th to 14th value of matrix1
matrix1[2:4, 1:3] #row 2 to 4 and column 1 to 3. if you don't sepcify the dimension names, R will just show 1,2,3 even if you choose row 2 to 4.
...
# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1 #;means after create list1, show list1
list1[[2]]
...
dataframe1 <- data.frame(d,e,f) # create data frame using d,e,f vectors
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # 

dataframe1[1:2,] # rows 1 and 2, and all the columns 
...
```{r DataExercise-1a}
vector4 <- c(64,71,72,63,51,52,61,64,67,73)
vector4
vector5 <- c(42,49,60,46,32,32,41,47,52,54)
vector5
```
...
```{r DataExercise-1b}
vector6 <- function(a){
  Celsius1 <- round((a-32)*5/9)
  return(Celsius1)
}
Celsius_High <- vector6(vector4)
Celsius_High

vector7 <- function(b){
  Celsius2 <- round((b-32)*5/9)
  return(Celsius2)
}
Celsius_Low <- vector7(vector5)
Celsius_Low
```
...
```{r DataExercise-1c}
df_temperature <- data.frame(vector5,Celsius_Low,vector4,Celsius_High)
df_temperature
names(df_temperature) <- c("FLow","CLow","FHigh","CHigh")
df_temperature
```
