...
```{r}
#Exercise 1:
plot1 = ggplot(EPAair, aes(x = Ozone, fill = as.factor(EPAair$Year))) + geom_density(alpha = 0.5) + scale_fill_manual(values = c("darkgreen", "darkblue")) + labs(fill = "", x = "Ozone AQI values", y = "Density")
print(plot1)

plot2 = ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) + geom_boxplot()+ stat_summary(geom = "text", fun = max, vjust = -1, size = 4, label = c("b", "a")) + labs(x = "", y = "Ozone AQI Value") + ylim(0,150)
print(plot2)

plot3 = ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) + geom_violin(draw_quantiles = 0.5)+ stat_summary(geom = "text", fun = max, vjust = -1, size = 4, label = c("b", "a")) + labs(x = "", y = "Ozone AQI Value") + ylim(0,150)
print(plot3)
```
...
plot4 = lm(data = EPAair, Ozone ~ PM2.5)
...
plot5 = ggplot(EPAair) + geom_point(aes(x = Ozone, y = PM2.5)) + geom_smooth(aes(x = PM2.5, y = Ozone), method = "lm", se = FALSE) 
...
library(corrplot)
plot6 = EPAair %>% select(mean_Lat:Ozone) %>% drop_na()
plot7 = cor(plot6)
corrplot(plot7, method = "ellipse")
corrplot.mixed(plot7, upper = "ellipse")
