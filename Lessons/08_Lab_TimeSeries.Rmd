...
Rainfall_BR_processed <-
  ...
  #seperate the statistics column to get the month data
  separate(Statistics,c("Null","Month","Null2")," ") %>% 
  # my: month year, paste0: Concatenate vectors after converting to character.
...
#create a sequence in your first and last, do a inner join, in time series, the missing data doesn't have rows in it, so we need to do the sequency and do the inner join
...
Rainfall_BR_ts = ts(BR_complete$Rainfall, start = c(1901, 1), frequency = 12) #see dataset, it's monthly data
Temperature_BR_ts = ts(BR_complete$Temperature_C, start = c(1901, 1), frequency = 12)

#more general
f_year = year(first(BR_complete$Date)) # the oldest date will return 
...
Rainfall_BR_De = stl(Rainfall_BR_ts, s.window = "periodic") #s.window: seasonal window
Temperature_BR_De = stl(Temperature_BR_ts, s.window = "periodic")
plot(Rainfall_BR_De)
plot(Temperature_BR_De)
#bars on the right means that zoom in. the bigger the bar is, the smaller impact it will have to the whole
...
Rainfall_BR_Components = as.data.frame(Rainfall_BR_De$time.series[,1:3])
#add two more columns
Rainfall_BR_Components = mutate(Rainfall_BR_Components, Observed = BR_complete$Rainfall, Date = BR_complete$Date)
Temperature_BR_Components = as.data.frame(Temperature_BR_De$time.series[,1:3])
Temperature_BR_Components = mutate(Temperature_BR_Components, Observed = BR_complete$Temperature_C, Date = BR_complete$Date)
