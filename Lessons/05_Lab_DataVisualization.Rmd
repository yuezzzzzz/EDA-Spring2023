...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    ...
    ) # paste: combine strings with space, paste0: without space between the two strings, R^{2}: mathjax
...
ggplot(PeterPaul.chem.nutrients) + geom_bar(aes(x = month)) #Histograms visualize quantitative data or numerical data, whereas bar charts display categorical variables
...
ggplot(PeterPaul.chem.nutrients, aes(x = factor(month, levels = 1:12, labels = month.abb))) + geom_bar() + scale_x_discrete(drop = FALSE)

ggplot(PeterPaul.chem.nutrients, aes(x = factor(month, levels = 1:12, labels = month.abb))) + geom_bar() + scale_x_discrete(drop = FALSE) + labs(title = "My Plot", x = "Month")
...
PeterPaul.chem.nutrients %>% filter(depth == 0) %>% ggplot() + geom_point(aes(x = daynum, y = temperature_C, color = year4), size = 0.5, alpha = 0.5) + facet_wrap(vars(lakename), nrow = 2)#alpha: transparency
