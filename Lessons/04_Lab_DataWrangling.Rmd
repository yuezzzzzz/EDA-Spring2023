...
NTL.phys.data.PeterPaul <- 
  filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # c()creating new vectors
...
NTL.phys.data.Tuesday <- filter(NTL.phys.data, lakename == "Tuesday Lake") # type the full name of the lake
NTL.phys.data.Tuesday.year <- filter(NTL.phys.data.Tuesday, year4 %in% c(1990:1999))
```
...
NTL.phys.data.Tuesday.year.july <- NTL.phys.data %>% filter(lakename == "Tuesday Lake" & year4 %in% c(1990:1999) & month(sampledate) == 7)
NTL.phys.data.Tuesday.year.july1 <- NTL.phys.data %>% mutate(month = month(sampledate)) %>% filter(lakename == "Tuesday Lake" & year4 %in% c(1990:1999) & month == 7)
...
NTL.phys.data.EX4 <- NTL.phys.data.Tuesday.year.july %>% filter(depth == 0) %>% drop_na(temperature_C) %>% summarize(meantemp = mean(temperature_C))
...
NTL.phys.data.gathered <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "Irradiance_Type", values_to = "Irradiance_Value")
NTL.phys.data.gathered1 <- NTL.phys.data %>% pivot_longer(irradianceWater:irradianceDeck, names_to = "Irradiance_Type", values_to = "Irradiance_Value")

#Exercise 6: Spread temperatureC into more than one column based on the depth.
NTL.phys.data.gathered2 <- NTL.phys.data %>% pivot_wider(names_from = depth, values_from = temperature_C)#normal to have so many NA, can use select function 
```
