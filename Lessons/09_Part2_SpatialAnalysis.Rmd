...
ggplot() + geom_sf(data = huc8_sf, fill = huc8_sf$ACRES) + scale_fill_distiller(palette = "Y10rRd")
...
upperneuse_sf = st_read("./Data/Spatial/NCHUC8.shp") %>% filter(huc8_sf$SUBBASIN == "Upper Neuse")


#Create a map
mapView(huc8_sf, col.regions = "orange") + mapView(upperneuse_sf, col.regions = "purple")

ggplot() + geom_sf(data = huc8_sf, fill = "purple", color = "grey") + geom_sf(data = upperneuse_sf, fill = "orange")
...
neuse_intersect_3 = counties_sf_utm %>% st_filter(neuse_sf, .pred = st_intersects)
#View the result
