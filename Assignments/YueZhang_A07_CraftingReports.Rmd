...
```{r project.setup, warning= FALSE, message= FALSE, error= FALSE, eval= TRUE}
#Import libraries
...
kable(summary_PM25, format = "simple", row.names = TRUE, 
      col.names = c("County", "$\\mu g/m^3$"), caption = "Mean Particulates (2.5mm)")

kable(summary_Ozone, format = "simple", row.names = TRUE, 
      col.names = c("County", "$\\mu g/m^3$"), caption = "Mean Ozone") 
```
...
```{r plots1, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Distribution of Ozone", fig.align='left', fig.height = 4, fig.width = 6}
library(gridExtra)
library(latex2exp)
Ozone2018 = epa_data %>% drop_na(Ozone) %>% filter(Year == 2018)
Ozone2019 = epa_data %>% drop_na(Ozone) %>% filter(Year == 2019)
par(mar = c(4,4,4,4))
Ozone2019_plot = ggplot(Ozone2019) + 
  geom_boxplot(aes(x = Month, y = Ozone)) + 
  labs(x = "Month(2018)", y = TeX("Ozone($\\mu g/m^3$)"))
Ozone2018_plot = ggplot(Ozone2018) + 
  geom_boxplot(aes(x = Month, y = Ozone)) + 
  labs(x = "Month(2019)", y = TeX("Ozone($\\mu g/m^3$)"))
grid.arrange(Ozone2018_plot, Ozone2019_plot, ncol=2)
...
### Git Repository
https://github.com/yuezzzzzz?tab=repositories
